...
3. Agora 状态层级
   ...
   - 第二行仅在 Agora 下展示 Profile、patch 数、verified/stale 和后台 intake activity。
...
4. MemoryPatch 状态
   ...
   - `/memory` 提供命名 Profile、Patch 多选、唯一 writable family、overlay、自动策略、历史和回滚。
   - 自动 intake 只推进 writable family，通过 CAS 保留 overlay；后台任务不得禁用输入框。
