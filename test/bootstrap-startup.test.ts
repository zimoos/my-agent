...
test('bootstrap hydration connects five MCP servers in parallel, preserves config order, and isolates failures', async () => {
  ...
  try {
    ...
    const config = {
      ...
      mcpServers: {
        alpha: { command: process.execPath, args: [slowA] },
        beta: { command: process.execPath, args: [slowB] },
        gamma: { command: process.execPath, args: [slowC] },
        broken: { command: process.execPath, args: [broken] },
        delta: { command: process.execPath, args: [slowD] },
        omega: { command: process.execPath, args: [slowE] },
      },
    };
    ...
    try {
      assert.ok(elapsed < 700, `parallel hydration took ${elapsed.toFixed(1)}ms`);
